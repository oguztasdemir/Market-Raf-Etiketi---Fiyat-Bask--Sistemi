...
import binascii
import subprocess
from src.config import MANAV_PRODUCTS_FILE, SCALE_SETTINGS_FILE, PRODUCTS_FILE, SISTEM_EXCELI_DIR
from src.utils.storage import load_json, save_json
from src.utils.text_cleaner import format_price_display, parse_price_val
...
def fetch_prices_from_scale(ip=None) -> dict:
    ...
    tools_dir = os.path.join(base_dir, "tools", "perkon")
    ...
    try:
        ...
        for i in range(total_blocks):
            ...
            try:
                ...
                name = ""
                m_name = re.search(r'0719([0-9A-F]{10,})', block)
                if m_name:
                    hex_str = m_name.group(1)
                    if "0D" in hex_str:
                        hex_str = hex_str.split("0D")[0]
                    try:
                        name = binascii.unhexlify(hex_str).decode("cp1254", errors="ignore").strip()
                    except:
                        name = ""
...
def export_scale_files(products: list = None) -> dict:
    ...
    """
    from src.config import SCALE_EXPORT_DIR
    if products is None:
    ...
    try:
        ...
        with open(dat_path, mode="wb") as f:
            for p in products:
                ...
                title = str(p.get("title", "")).strip()[:30]
                cents = _parse_price_to_cents(p.get("price", "0"))
                line = f"{plu:06d}01{cents:08d}{title:<30}\r\n".encode("cp1254", errors="ignore")
                f.write(line)
